...
function tagTemplate(tags) {
    return tags.map((tag) => `<p class="food-classification">${tag}</p>`).join(" ");
}
...
function recipesTemplate(recipes) {
    return `<div class = "recipe_info">
        <img src="${recipes.image}" alt="${recipes.name} image">
        <div class="food-info">
            <div class="tags">${tagTemplate(recipes.tags)}</div>
            <h2>${recipes.name}</h2>
            <div class=rating>${ratingTemplate(recipes.rating)}
            </div>
            <p class="hide">${recipes.description}</p>
        </div>
    </div>`
}
